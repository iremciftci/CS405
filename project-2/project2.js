...
class MeshDrawer {
	...
	constructor() {
		 ...
		 */
		this.locatelightpos = gl.getUniformLocation(this.prog, 'PosLight');
        this.NormalLocate = gl.getAttribLocation(this.prog, 'normal');
		this.PosLight = [1.0, 1.0, 1.0]; 
	...
	draw(trans) {
		...
		///////////////////////////////
		gl.bindBuffer(gl.ARRAY_BUFFER, this.normalbuffer);
        gl.enableVertexAttribArray(this.NormalLocate);
        gl.vertexAttribPointer(this.NormalLocate, 3, gl.FLOAT, false, 0, 0);
...
const meshFS = `
			...
			void main() {
				...
				if (enableLighting) {
					...
					vec3 lightDirection = normalize(PosLight); // Calculate the light direction
